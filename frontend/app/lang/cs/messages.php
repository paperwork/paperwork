<?php

return array(

	'account_creation_failed' => 'Could not create account.',
	'account_update_failed' => 'Could not update account.',
	'invalid_credentials' => 'Your credentials are invalid.',
	'note_version_info' => 'You are previewing an older version of this note.',
	'found_bug' => 'Found a bug? <a href="https://github.com/twostairs/paperwork/issues/new" target="_blank" title="Submit Issue">Submit it on GitHub!</a>',
	'new_version_available' => 'Found a bug? It seems like you are not running the latest version of Paperwork. Please consider updating before submitting an issue. ',
	'error_version_check' => 'Found a bug? Paperwork cannot check whether your version is the latest. Please make sure you are running the latest version before reporting any issues. ', 
	'error_message' => 'Whooops!',
	'onbeforeunload_info' => 'Data will be lost if you leave the page, are you sure?',
	'user' => array(
		'settings' => array(
			'language_label' => 'Language',
			'client_label' => 'Client',
			'import_slash_export' => 'Import/Export',
			'language' => array(
				'ui_language' => 'Jazyk uživatelského rozhraní',
				'document_languages' => 'Jazyky dokumentů',
				'document_languages_description' => 'Jazyky, jež vyberete, budou použity ke zpracování textu ve vámi nahraných přílohách. Budete tak v nich moci prohledávat jejich obsah. Přílohou může například být fotografie dokumentu, kterou pořídíte svým telefonem. Vyberte jazyky, v nichž jsou tyto dokumenty obvykle psány.',
			),
			'client' => array(
				'qrcode' => 'QR kód',
				'scan' => 'Sejměte tento QR kód pomocí mobilní aplikace pro automatické nastavení vašeho účtu pro Paperwork.'
			),
			'import' => array(
				'evernotexml' => 'Evernote XML:',
				'upload_evernotexml' => 'Zde nahrajte data XML vyvedená z Evernote, aby byly vaše poznámky vytvořené v Evernote zavedeny do Paperwork.'
			),
			'export' => array(
				'evernotexml' => 'Evernote XML:',
				'download_evernotexml' => 'Stáhněte soubor ENEX slučitelný s Evernote pro přesun vašich poznámek z Paperwork. '
			)
		)
	)
);
