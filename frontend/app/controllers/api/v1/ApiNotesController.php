...
class ApiNotesController extends BaseController
{
    ...
    public function search($queryBase64Encoded)
    {
        ...
        $dateFilter =
          preg_match_all("/\s*date\:((?:19|20)\d\d)(?:[-\.\/](0[1-9]|1[012])(?:[-\.\/](0[1-9]|[12][0-9]|3[01]))?)?/",
        ...
        if ($dateFilter !== false && $dateFilter > 0) {
            ...
            $notes =
              $notes->where(DB::raw('YEAR(notes.updated_at)'), '=', $date[1]);
            if (isset($date[2])) {
                $notes = $notes->where(DB::raw('MONTH(notes.updated_at)'), '=',
                  sprintf("%02s", $date[2]));
            }
            if (isset($date[3])) {
                $notes = $notes->where(DB::raw('DAY(notes.updated_at)'), '=',
                  sprintf("%02s", $date[3]));
            }
        ...
        $filters =
          preg_match_all("/\s*(tagid|note(?:book)?id)\:(\d+)/", $searchQuery,
        ...
        if ($filters !== false && $filters > 0) {
            foreach ($matches as $match) {
                switch ($match[1]) {
                    ...
                    case "notebookid" :
                        $notes =
                          $notes->where('notes.notebook_id', '=', $match[2]);
                        break;
                }
            }
        ...
        $notes = $notes->whereNull('notes.deleted_at')
                       ...
                       ->select('notes.id', 'notes.notebook_id',
                         'notebooks.title as notebook_title', 'versions.title',
                         'versions.content_preview', 'versions.content',
    ...
    {
        $noteIds   =
          explode(PaperworkHelpers::MULTIPLE_REST_RESOURCE_DELIMITER, $noteId);
        $responses = array();
